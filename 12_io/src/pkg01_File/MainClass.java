...
public class MainClass {
  ...
  public static void method1() {
    ...
    // File 객체 선언
     ...
     // 디렉터리 유무에 따른 생성
     if(/*!dir.exists()*/ dir.exists()==false) {  // 디렉터리가 없으면
       dir.mkdirs();
     } else {                                     // 디렉터리가 있으면
  ...
  public static void method3() {
    ...
    /*
     * File 클래스 주요 메소드
     * 1. liestFiles()   : 디렉터리 내의 모든 File 객체를 File[] 배열로 반환
     * 2. getName()      : 이름 반환
     * 3. getParent()    : 저장된 디렉터리 반환
     * 4. getPath()      : getParent() + getName()
     * 5. lastModified() : 최종수정일을 long 타입으로 반환(TimeStamp : 숫자 하나 딱! 알아서 변환해서 써라)
     * 6. length()       : 파일 크기를 long 타입으로 반환 (바이트 반환)
  ...
  public static void practice1() { 
    ...
    File dir = new File(sdf.format(timestamp));
    if(!dir.exists()) {
      dir.mkdirs();
    }
    */
  ...
  public static void practice2(File dir) {
    ...
    // C:\2024\01\22 디렉터리 삭제하기(비어있는 디렉터리만 삭제할 수 있다.)
    ...
    // 현재 디렉터리 삭제
    System.out.println(dir.getPath() + "삭제함");
    dir.delete();
    
    // 상위 디렉터리를 practice2 메소드에 다시 전달
    practice2(parent);
    
    
    /*
    File home = new File("\\2024");
