...
public class MainClass {
   ...
   * 2. 오라클에서 만들어서 배포한다.
   * 3. 18c XE 버전의 경우 C:\app\사용자\product\18.0.0\dbhomeXE\jdbc\lib\ojdbc8.jar 파일을 프로젝트의 build path 에 등록해서 사용한다.
   */
  ...
  public static void method1() {
    ...
    try {
      Class.forName("oracle.jdbc.OracleDriver");    // 오타가 나면 오류가 날 수 있음. try-catch 안에 꼭 넣어주기
      System.out.println("성공");
    } catch (Exception e) {
      System.out.println("oracle.jdbc.OracleDriver 클래스를 찾을 수 없다.");
    }
  ...
  public static void method4() {
    ...
    try {
      con = DriverManager.getConnection(url, user, password);
      System.out.println("데이터베이스 접속되었습니다.");
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
      try {
